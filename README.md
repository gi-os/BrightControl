...
| --- | --- |
| v2.16 | **The lock face shows the notifications that fit, and scrolls to the rest.** The third one always had its bottom half cut off: the column was a plain list at the end of a weighted layout, so it was handed whatever space was left over and drew past it — and no count of notifications can fix that, since how many fit depends on how many lines each of them has. The column now measures its rows and ends where a row ends, so nothing is ever half-drawn, and a drag inside it moves through the rest, settling on a boundary. It scrolls only when there is something to scroll, so **swipe up for the keypad still works over the notifications** — the drag never asks for the bouncer and never unlocks anything. Up to eight are reachable, `+N MORE` past that, and the column is rebuilt only when the notifications themselves change rather than on every minute tick |
