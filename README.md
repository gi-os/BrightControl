...
|---------|--------------|
| v4.29 | **Wi-Fi login: the handoff was arriving and nobody was home.** Under a VPN the log ended `fired the system's sign-in notification → true`, `onStart: rebound → false`, and the same screen sat there. The notification opened *this app* — and the screen is `singleTask`, so Android calls `onNewIntent` rather than building it again, with the `CaptivePortal` binder, the network and the portal URL on the intent. There was no `onNewIntent`, so v4.28's whole fix never got its extras. It listens now and spends them at once. A fired notification is no longer taken at its word either: Android's page coming up stops this activity and the round trip lands in `onNewIntent`, so neither happening within 2.5s means the notification went nowhere and the direct component launch is taken without asking |
...
| v4.27 | **GRANT ALL sets up the neighbours.** LightOS has no page for any of these and the platform keeps them behind secure settings only the shell can write: `credential_service` and `credential_service_primary` name Bitwarden's credential provider, so a passkey request from Web Tools is answered by the vault; `autofill_service` gives app passwords to Bitwarden too; and `cmd role add-role-holder android.app.role.BROWSER` makes Web Tools the browser every link opens in. Each step is skipped when its app is absent and read back like the rest, so the screen says OK, FAILED or UNKNOWN per line |
...
| v4.15 | **The login page names the VPN that is stopping it.** First reports from v4.14 (light-reports #242–#244) showed the join working — the phone on the portal network, LightOS flagging it CAPTIVE_PORTAL, WebView present — and then `bindProcessToNetwork → false` and `EPERM` on every probe, with a VPN (`tun0`) as the default network. netd forbids a UID under a VPN from selecting any other network, and no app can lift that. The portal screen now recognises the case: it says a VPN is on (naming the always-on app when the phone will), offers OPEN VPN SETTINGS, and stops probing; the Wi-Fi screen shows the same row under THIS NETWORK. A bind failure with no VPN is reported separately |
| v4.14 | **Wi-Fi joins without the shell.** v4.13 failed on its first outing — *the connection is gone and could not be picked back up* — because wireless debugging only runs while the phone is already on Wi-Fi, so the shell can never join the first network. Off Wi-Fi the screen now hands the network to Android as a `WifiNetworkSuggestion` and Android joins it; scanning falls back to `WifiManager` (fine location + location on, both named when missing). Suggestions need a one-time per-app approval Android asks through a notification LightOS has no shade for, so: `cmd wifi network-suggestions-set-user-approved` is a SelfGrant step (checked with the new `GrantCheck.ShellSays`), the APPROVAL row runs it whenever the shell is reachable, and the notification listener finds the system's "Allow" button and the screen presses it. The shell route stays for when on Wi-Fi (instant join, save, forget) |
| v4.13 | **Wi-Fi: join the networks Settings calls "not supported".** LightOS Settings answers an open network with *This network is not supported by The Light Phone* — a rule in Light's Settings app, not in the radio. The Wi-Fi login screen is now a Wi-Fi screen: it scans and joins over the phone's own shell (`cmd wifi start-scan` / `list-scan-results` / `connect-network <ssid> open|owe|wpa2|wpa3 [pass]`), the same wireless-debugging shell the grants use, with signal bars, band and security per network, a password field for WPA2/WPA3, saved networks with FORGET, and Wi-Fi on/off. After a join it watches the system's verdict: VALIDATED is done, anything else after 12 s is a login page and the portal screen opens by itself. Parsers are pure and unit-tested; enterprise (EAP) and WEP are shown but not joinable |
| v4.12 | **The Wi-Fi login screen keeps a log and reports its own failures.** It fails on the phone and nobody could say why — one status line was the whole record. Now everything it learns is written down (WebView provider, LightOS's `captive_portal_*` settings, every network and its VALIDATED/CAPTIVE_PORTAL bits, the bind result, every probe's HTTP code and redirect, every page the WebView starts, finishes or errors on) and the failures it can recognise — no WebView, no Wi-Fi, bind refused, page not loaded in 25 s, five probes throwing in a row, renderer dead — file that log to light-reports by themselves, once per opening and at most one every ten minutes. LOG shows it on the phone, SEND LOG files it by hand, a portal whose certificate the WebView refuses now offers LOAD IT ANYWAY instead of a blank page, and the settings screen says up front whether this phone has a WebView and whether LightOS turned login-page detection off |
| v4.11 | **The switcher gets its screen back.** It had drifted to two apps on the LPIII: every piece of furniture the list gained since v3.97 — the HOME heading, a wider way-out button — was subtracted from the row budget, each correct alone, until the floor clamp was quietly deciding the answer and the pinned Home and current-app rows were paid out of *it*. The air around rows and furniture is trimmed to what the panel can afford, the arithmetic now lives in a pure `SwitcherFit` object, and a JVM test holds it against the 1080×1240 panel: five rows, pinned Home included, or the build fails |
| v4.10 | **Today's weather on the lock face, one dim line.** `72° · CLEAR · H 81 L 64` under the status bar, read from LightFog's weather provider — no location, no radio, no poll; absent rather than stale past three hours |
| v4.9 | **A withdrawn banner stays withdrawn, and a pocketed tap can't light the torch.** An app that retracts a notification now retracts its banner too, and tap/hold timers are swept when the screen goes off so the wheel's default Torch tap can no longer fire in a pocket. Fast service toggles keep their callbacks straight |
